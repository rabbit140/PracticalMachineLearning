...
###Random forest
The model fitting command was run, but after 30 minutes of fitting the random forest, it was halted by the user as itwas concluded that fitting it was too demanding for an i3-4010U mobile CPU.
The overall accuracy is quite low (~50%), let's try fitting a random forest:
...
The accuracy seems to have improved by over 20% in comparison to classification tree.
...
Fitting a QDA model resulted in further 20% accuracy improvement, giving a total of 40% better accuracy than a classification tree. It is thus clear that QDA should be used to make predictions on the test data.